<template>
    <div class="comfyui-body grid h-full w-full overflow-hidden">
        ...
        <div v-show="!dialogStore.aiAppEditMode" id="comfyui-body-bottom" class="comfyui-body-bottom">
            <TopMenubar v-if="useNewMenu === 'Bottom'" />
        </div>
        <div v-show="!dialogStore.aiAppEditMode" id="comfyui-body-left" class="comfyui-body-left" />
        <div v-if="dialogStore.aiAppEditMode" id="ai-app-edit-panel" class="ai-app-edit-panel">
            <AiAppEditPanel />
        </div>
...
import AiAppEditPanel from "@/components/dialog/AiAppEditPanel.vue";
...
if (isElectron()) {
    watch(
        ...
        (newTasks, oldTasks) => {
            ...
            newTasks
                .filter((task) => oldRunningTaskIds.has(task.promptId) && task.isHistory)
                .forEach((task) => {
...
const onGraphReady = () => {
    requestIdleCallback(
        () => {
            ...
            void wrapWithErrorHandlingAsync(useNodeFrequencyStore().loadNodeFrequencies)();
